...
export async function updateDashboardTimezones(timezones: string[]): Promise<string[]> {
  const row = await request<{ timezones: string[] }>("/dashboard/timezones", {
    method: "PATCH",
    body: JSON.stringify({ timezones }),
  });
  return row.timezones;
}
