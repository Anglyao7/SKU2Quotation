          ...
          <table className="admin-data-table admin-inquiry-table">
            ...
            <tbody>{pageRows.map((row) => <tr key={row.id}>
              ...
              <td><div className="admin-row-actions">
                {canRevise && !row.readOnly && ["PENDING_CONFIRMATION", "CONFIRMED"].includes(row.status) ? <Button asChild size="2"><Link to={quoteDocumentHref(row.id, "quotation")}>{t(canUseExtendedDocuments ? "前往多证工作台" : "报价工作台")}</Link></Button> : null}
