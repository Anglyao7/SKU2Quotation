...
export function QuotesPage() {
  ...
  return <div className="core-workspace">
    ...
    <Tabs.Root defaultValue="public">
      <Tabs.List><Tabs.Trigger value="public">{t("客户前台草稿")} ({publicDrafts.length})</Tabs.Trigger><Tabs.Trigger value="official">{t("正式报价")} ({quotes.length})</Tabs.Trigger></Tabs.List>
      <Tabs.Content value="public"><Card className="core-notice"><ShoppingCartSimple /><div><Text weight="bold" as="div">{t("客户前台询价")}</Text><Text size="2" color="gray">{t("确认后，访客会在前台个人中心收到更新提醒。")}</Text></div></Card><div className="core-draft-grid">{publicDrafts.map((draft) => <Card className="core-draft-card" key={draft.id} onClick={() => void openPublicDraft(draft.id)}><div className="core-panel-heading"><div><Text size="1" color="gray">{draft.quoteNumber}</Text><Heading size="4">{draft.customerCompany || draft.customerName}</Heading></div><Badge color={draft.status === "COMPLETED" ? "jade" : draft.status === "CONFIRMED" ? "blue" : draft.status === "CANCELLED" ? "gray" : "amber"}>{t(label(draft.status))}</Badge></div><Text size="2" color="gray">{t("联系人")}：{draft.customerName} · {t("国家")}：{countryLabel(draft.visitorCountryCode)}</Text><strong>{draft.currency} {draft.total.toFixed(2)}</strong><Text size="1" color="gray">{t("提交于 {created} · 有效至 {valid}", { created: coreDate(draft.createdAt), valid: coreDate(draft.validUntil) })}</Text><Button variant="soft">{t("查看详情")}</Button></Card>)}{!loading && !publicDrafts.length ? <CoreEmpty title={t("暂无客户前台草稿")} description={t("客户在店铺提交报价请求后，会进入这里等待人工确认。")} /> : null}</div></Tabs.Content>
      <Tabs.Content value="official"><Card className="core-table-card"><div className="core-table core-quotes-table"><div className="core-table-head"><span>{t("报价编号 / 客户")}</span><span>{t("版本")}</span><span>{t("金额")}</span><span>{t("审批门禁")}</span><span>{t("状态")}</span><span>{t("更新时间")}</span></div>{quotes.map((quote) => <button type="button" className="core-table-row" key={quote.id} onClick={() => void openQuote(quote.id)}><span className="core-name-cell"><span className="core-row-icon"><FileText /></span><span><strong>{quote.customerName}</strong><small>{quote.quotationNumber}{quote.readOnly ? ` · ${t("主账号只读")}` : ""}</small></span></span><strong>v{quote.currentVersion}</strong><strong>{quote.currency} {quote.totalAmount.toFixed(2)}</strong><span><ShieldCheck />{t("人工")}</span><Badge color={quote.status === "APPROVED" ? "jade" : quote.status === "REJECTED" ? "red" : "amber"}>{t(label(quote.status))}</Badge><span>{coreDate(quote.updatedAt)}</span></button>)}{loading && !quotes.length ? <CoreLoading label={t("正在读取报价版本")} /> : null}{!loading && !quotes.length ? <CoreEmpty title={t("尚无正式报价")} description={t("从询盘工作台人工选择候选并创建报价。")} /> : null}</div></Card></Tabs.Content>
    </Tabs.Root>
