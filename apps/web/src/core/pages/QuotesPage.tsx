...
export function QuotesPage() {
  ...
  return <div className="core-workspace">
    ...
    <Tabs.Root defaultValue="official">
      ...
      <Tabs.Content value="public"><Card className="core-notice"><ShoppingCartSimple /><div><Text weight="bold" as="div">{t("客户前台提交，待内部人工确认")}</Text><Text size="2" color="gray">{t("这些记录只保存客户意向和当时的商品快照，不等于正式 Quotation，也不能直接发送为承诺。")}</Text></div></Card><div className="core-draft-grid">{publicDrafts.map((draft) => <Card className="core-draft-card" key={draft.id} onClick={() => void openPublicDraft(draft.id)}><div className="core-panel-heading"><div><Text size="1" color="gray">{draft.quoteNumber}</Text><Heading size="4">{draft.customerCompany || draft.customerName}</Heading></div><Badge color="amber">{t(label(draft.status))}</Badge></div><Text size="2" color="gray">{t("联系人")}：{draft.customerName}</Text><strong>{draft.currency} {draft.total.toFixed(2)}</strong><Text size="1" color="gray">{t("提交于 {created} · 有效至 {valid}", { created: coreDate(draft.createdAt), valid: coreDate(draft.validUntil) })}</Text><Button variant="soft">{t("查看客户提交快照")}</Button></Card>)}{!loading && !publicDrafts.length ? <CoreEmpty title={t("暂无客户前台草稿")} description={t("客户在店铺提交报价请求后，会进入这里等待人工确认。")} /> : null}</div></Tabs.Content>
    </Tabs.Root>
