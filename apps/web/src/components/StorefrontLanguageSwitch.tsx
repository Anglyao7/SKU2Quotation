...
import { useStorefrontLanguageTransition } from "./StorefrontLanguageTransition";
...
export function StorefrontLanguageSwitch({
  ...
  const { active: transitionActive, begin, complete } = useStorefrontLanguageTransition();
  ...
  const selectLocale = (nextLocale: StorefrontLocale) => {
    if (nextLocale === locale || transitionActive) return;
    begin(nextLocale);
    onBeforeLocaleChange?.();
