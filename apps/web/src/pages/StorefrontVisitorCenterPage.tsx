  ...
  return <div className="visitor-quote-list">{quotes.map((quote) => (
    <Card className="visitor-quote-row" key={quote.id}>
      <div className="visitor-quote-heading">
        ...
        <Badge color={quote.status === "COMPLETED" ? "jade" : quote.status === "CONFIRMED" ? "blue" : "amber"}>
          {t(quote.status === "COMPLETED" ? "已成交" : quote.status === "CONFIRMED" ? "商家已确认" : "待商家确认")}
        </Badge>
      </div>
...
export function StorefrontVisitorCenterPage() {
  ...
  const loadQuotes = useCallback(async () => {
    ...
    try {
      ...
      markQuoteNotificationsSeen(
        store.slug,
        rows.filter((row) => row.status === "CONFIRMED" || row.status === "COMPLETED").map(quoteNotificationKey),
      );
    } catch (reason) {
  ...
  return <div className="store-shell visitor-center-shell" dir={storefrontDirection(locale)}>
    ...
    <main className="visitor-center-main"><Container size="4">
      ...
      <Tabs.Root defaultValue="history" className="visitor-center-tabs">
        <Tabs.List>
