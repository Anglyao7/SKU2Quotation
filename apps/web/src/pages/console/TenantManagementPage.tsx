      ...
      <Dialog.Content>
        ...
        {result ? (
          ...
          <form className="dialog-form" onSubmit={submit} key={tenant?.id}>
            ...
            <label className="field-group"><Text size="2" weight="medium">租户角色 *</Text><Select.Root name="role" defaultValue="OWNER"><Select.Trigger /><Select.Content><Select.Item value="OWNER">所有者（OWNER）</Select.Item><Select.Item value="ADMIN">管理员（ADMIN）</Select.Item><Select.Item value="SALES">销售编辑（SALES）</Select.Item><Select.Item value="PURCHASING">采购编辑（PURCHASING）</Select.Item><Select.Item value="VIEWER">只读成员（VIEWER）</Select.Item></Select.Content></Select.Root></label>
...
function TenantFormDialog({ tenant, onOpenChange, onSaved }: { tenant: Tenant | "new" | null; onOpenChange: (open: boolean) => void; onSaved: () => Promise<void> }) {
  ...
  return (
    <Dialog.Root open={Boolean(tenant)} onOpenChange={onOpenChange}><Dialog.Content><Dialog.Title>{current ? "编辑商家" : "新增商家"}</Dialog.Title><Dialog.Description>商家拥有独立商品前台和租户数据空间；前台地址会根据商家名称自动生成。</Dialog.Description><form className="dialog-form" onSubmit={submit} key={current?.id || "new"}><label className="field-group"><Text size="2" weight="medium">商家名称 *</Text><TextField.Root name="name" required defaultValue={current?.name} placeholder="例如 海岸家居" /><Text size="1" color="gray">中文名称可直接用于前台路径；修改名称后，旧地址仍会自动跳转。</Text></label><label className="field-group"><Text size="2" weight="medium">联系邮箱</Text><TextField.Root name="contact_email" type="email" defaultValue={current?.contact_email || ""} placeholder="ops@company.com" /></label><label className="field-group"><Text size="2" weight="medium">状态</Text><Select.Root name="status" defaultValue={current?.status || "active"}><Select.Trigger /><Select.Content><Select.Item value="active">启用</Select.Item><Select.Item value="inactive">停用</Select.Item></Select.Content></Select.Root></label>{error && <Callout.Root color="red"><Callout.Icon><WarningCircle /></Callout.Icon><Callout.Text>{error}</Callout.Text></Callout.Root>}<div className="dialog-actions"><Dialog.Close><Button variant="soft" color="gray">取消</Button></Dialog.Close><Button type="submit" loading={saving}>保存商家</Button></div></form></Dialog.Content></Dialog.Root>
  );
}
