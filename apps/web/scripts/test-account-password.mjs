...
assert.match(pageSource, /\{view === "merchant" && canManageMerchant \? <Card className="account-merchant-card">/);
assert.match(appSource, /path:\s*"storefront\/brand"[\s\S]*?<AccountSettingsPage view="merchant" \/>/);
assert.match(layoutSource, /<DropdownMenu\.Root>/);
assert.match(layoutSource, /to="\/console\/account"/);
assert.match(layoutSource, /退出登录/);
assert.match(
  layoutSource,
  /isCustomerSubaccount\s*\?\s*<Text[^>]*>\{displayName\}<\/Text>/,
);
