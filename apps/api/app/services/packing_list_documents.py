...
from .public_quote_documents import _apply_xlsx_grid_borders, _pdf_localized_text, _place_quote_image, _quote_custom_fields, _quote_custom_value, _quote_pdf_image, _register_quote_pdf_font, _xlsx_text
...
def render_packing_list_pdf(document, *, image_loader=None):
    ...
    weights = [14, 39, 23, 29, 18, 29, 20, 20, 15, 20, 22, 25, *([24] * len(custom_fields))]
    widths = [pdf.width*w/sum(weights) for w in weights]
    cells = [[paragraph(label, white) for label in [*headers(locale), *[str(field.label) for field in custom_fields]]]]
    for item, row in zip(quote.items, rows, strict=False):
        cells.append([*[_quote_pdf_image(row["image_url"], image_loader, max_width=widths[0]-6, max_height=40) if field == "image_url" else paragraph(row[field]) for field in FIELDS], *[paragraph(_quote_custom_value(field, item)) for field in custom_fields]])
    footer = [quote_text(locale, "total"), "", "", "", "", "", "", "", total(rows, "carton_count"), total(rows, "quantity"), total(rows, "total_volume"), total(rows, "total_gross_weight"), *([""] * len(custom_fields))]
...
def render_packing_list_xlsx(document, *, image_loader=None):
    ...
    for row_number, text in [(1, title), (2, settings.packing_list_number + " · " + settings.issue_date.isoformat()), (3, proforma_text(locale, "seller") + ": " + party_text(parties["seller"], locale, "seller")), (4, proforma_text(locale, "buyer") + ": " + party_text(parties["buyer"], locale, "buyer"))]:
        sheet.merge_cells(start_row=row_number, start_column=1, end_row=row_number, end_column=12 + len(custom_fields))
    ...
    sheet.append([*headers(locale), *[str(field.label) for field in custom_fields]])
    for cell in sheet[5]:
    ...
    for item, row in zip(quote.items, rows, strict=False):
        sheet.append([*[None if field == "image_url" else float(row[field]) if isinstance(row[field], Decimal) else _xlsx_text(row[field]) if isinstance(row[field], str) else row[field] for field in FIELDS], *[_quote_custom_value(field, item) for field in custom_fields]])
    ...
    sheet.append([quote_text(locale, "total"), None, None, None, None, None, None, None, *[float(value) if value is not None else None for value in (total(rows, "carton_count"), total(rows, "quantity"), total(rows, "total_volume"), total(rows, "total_gross_weight"))], *([None] * len(custom_fields))])
    ...
    if settings.remarks:
        ...
        sheet.merge_cells(start_row=sheet.max_row, start_column=1, end_row=sheet.max_row, end_column=12 + len(custom_fields))
    ...
    for index, width in enumerate([14, 32, 22, 20, 16, 26, 17, 17, 13, 16, 19, 20, *([22] * len(custom_fields))], 1):
        sheet.column_dimensions[get_column_letter(index)].width = width
    sheet.freeze_panes = "E6"
    sheet.auto_filter.ref = f"A5:{get_column_letter(12 + len(custom_fields))}{5 + len(rows)}"
