...
> 状态：v2.5 独立知识库与智能体绑定架构已实现 + 后续路线图
> 版本：2.5
> 日期：2026-08-15
...
- 知识库管理：`/console/agents/knowledge` 只展示知识库列表，点击后进入
  `/console/agents/knowledge/:knowledgeBaseId` 详情页。先选择智能体，再选择或创建知识库。
  一个知识库只绑定一个智能体和一个店铺；一个智能体可以拥有多个知识库。文件上传、解析、批准、来源
  列表和训练数据都只作用于当前知识库。普通 PDF、DOCX、TXT、Markdown、JSON 进入事实知识
  处理；`support-ai-training/v1` 案例 JSON 在当前知识库内解析为训练草稿。
- AI 训练工作台：从当前知识库进入
  `/console/agents/{agent_id}/training?knowledge_base_id={knowledge_base_id}`，仅保留“案例训练”
  和“复用规则”。可人工增删改、导出，并通过“一键审批”统一批准和立即生效；不调用模型
  生成案例或规则。旧的智能体级训练行保留为所有知识库的兼容回退，不影响新知识库隔离。
...
10. 文件知识副本同时记录 `tenant_id`、`agent_id` 与 `knowledge_base_id`。店铺更换智能体后，运行时只检索当前
    智能体且状态为 ACTIVE 的知识库，旧智能体文件不会混入新智能体回答；同一智能体的多个
    知识库也相互隔离，可分别上传文件、训练和发布版本。
...
`20260813_0083_support_ai_training.py`、`20260815_0086_support_ai_knowledge_bases.py`。后续章节同时保留长期目标；标为 Phase 5/6
的能力不属于本次 v1 自动回答范围。
